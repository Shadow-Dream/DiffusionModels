...
class UNet(nn.Module):
    def __init__(
        ...
        self.down_conv_31 = ResidualBlock(channels*4, channels*8,channels*4, num_classes)
        self.down_down_3 = Downsample(channels*8)

        self.mid_conv_0 = ResidualBlock(channels*8, channels*8,channels*4, num_classes)#8
        self.mid_conv_1 = AttentionResidualBlock(channels*8, channels*8,channels*4, num_classes)
        self.mid_conv_2 = ResidualBlock(channels*8, channels*8,channels*4, num_classes)

        self.v_conv_0 = AttentionResidualBlock(channels*8, channels*8,channels*4, num_classes)
        self.v_dense_0 = nn.Linear(channels*8, channels)
        self.v_dense_1 = nn.Linear(channels, 1)

        self.up_up_0 = Upsample(channels*8)
        self.up_conv_00 = ResidualBlock(channels*16, channels*8,channels*4, num_classes)#16
        self.up_conv_01 = ResidualBlock(channels*8, channels*4,channels*4, num_classes)

        self.up_up_1 = Upsample(channels*4)
        self.up_conv_10 = AttentionResidualBlock(channels*8, channels*4,channels*4, num_classes)#32
        self.up_conv_11 = AttentionResidualBlock(channels*4, channels*2,channels*4, num_classes)

        self.up_up_2 = Upsample(channels*2)
        self.up_conv_20 = ResidualBlock(channels*4, channels*2,channels*4, num_classes)#64
        self.up_conv_21 = ResidualBlock(channels*2, channels,channels*4, num_classes)

        self.up_up_3 = Upsample(channels)
        self.up_conv_30 = ResidualBlock(channels*2, channels,channels*4, num_classes)#64
    ...
    def forward(self, x, t, y):
        ...
        v0 = self.v_conv_0(mid2,t,y)
        v1 = torch.mean(torch.mean(v0,-1),-1)
        v2 = self.v_dense_0(v1)
        v3 = nn.functional.relu(v2)
        v4 = self.v_dense_1(v3)
        v_out = (nn.functional.sigmoid(v4)+1)/2
        return noise_out,v_out